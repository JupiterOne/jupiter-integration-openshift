import { SERVICE_ACCOUNT_ENTITY_CLASS, SERVICE_ACCOUNT_ENTITY_TYPE, ServiceAccountEntity } from "../../jupiterone";
import { NamespaceData } from "../../openshift/types";
import { generateEntityKey } from "../../utils/generateKeys";

export function createServiceAccountEntities(namespaces: NamespaceData[]): ServiceAccountEntity[] {
  const defaultEntities: ServiceAccountEntity[] = [];

  return namespaces.reduce((namespaceSAEntities, namespace) => {
    const srvAccEntities = namespace.serviceAccounts.reduce((srvAccs, srvAcc) => {
      const srvAccEntity: ServiceAccountEntity = {
        _class: SERVICE_ACCOUNT_ENTITY_CLASS,
        _key: generateEntityKey(SERVICE_ACCOUNT_ENTITY_TYPE, srvAcc.metadata.uid),
